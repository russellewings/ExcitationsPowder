...
%
% `'s_rng'`
% : Provides a random number generator seed (obtained by using s=rng in 
%   Matlab, see <help rng> for details). Provides the same seed for the random
%   number generator that sets the Q on which the powder calculation is done. 
%   Set this value if you are fitting and hence want to calculate using the same
%   Q-points for every iteration
%
...
inpForm.fname  = [inpForm.fname  {'fid' 's_rng'}];
inpForm.defval = [inpForm.defval {-1    0      }];
...
if ~isstruct(param.s_rng) && param.s_rng==0
    s=rng;
elseif isstruct(param.s_rng) && isfield(param.s_rng,'Type') && isfield(param.s_rng,'Seed') && ...
        isfield(param.s_rng,'State')
    rng(param.s_rng);
    s=param.s_rng;
else
    s=rng;
